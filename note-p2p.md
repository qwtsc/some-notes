...
### p2p内容分发
现在大多数的p2p系统都属于内容分发的类别中，通过p2p内容节点的共同合作，共享资源，可以就代替需要使用高性能服务器的需求。
* p2p虚拟网络的一些高级特性能帮助p2p性能的提升，但是也需要注意这些高级特性带来的overhead与实际性能的提升需要进行权衡。CDN的p2p系统需要首先考虑物理位置（locality awareness）以达到快速地响应用户请求，同时也需要注意的是如何简单且可接受地维护一个p2p系统来适应节点动态性和大规模特性。
...
* 结构化地虚拟网络和非结构化虚拟网络的结合对于CDN来说也是很有意思但有挑战性的，有一个想法就是可以使用一种虚拟网络overlay来对另一种overlay来进行维护。

#### p2p CDN解决方案
* 书中主要使用了CoralCDN作为案例来详细介绍。CoralCDN依赖于多层次树状结构化虚拟网络架构，每一层都由树状虚拟网络构成，不同层次中虚拟网络所要求的节点之间的RTT最小范围不同，例如一个三层网络结构下，第一层的最小RTT要求是无穷大，第二层是60ms，第三层是20ms，那么如果两个节点都处于第三层，则两者的空间位置最为接近，另外一个节点可以隶属于不同层的虚拟网络，但是在所有层中的标识是保持不变的。
* 当客户端发出请求后，根据RTT的测量结果，DNS服务会把请求转移至RTT最小的CoralCDN节点，由该节点作为代理来发出请求，同时也由该代理将请求结果发送回客户端。首先，该代理节点检查本地的缓存是否有该请求内容，如果有那么返回，如果没有则开始使用结构化网络的请求路由服务，从最高层的虚拟网络开始找起（因为最高层RTT最小），直到找到对应的请求对象，之后再获取该对象并在代理节点中缓存，返回给客户端。之后，该代理节点将自身的节点标识与该请求对象的key之间的映射也插入DHT中，那么后续请求该项内容就直接可以返回之前的内容。但是CoralCDN并不会将客户端纳入整个系统中来，只能通过投资更多的代理服务器来提高可扩展性。
* 文中还穿插介绍了中心化方法来告诉客户端应该和哪些节点进行连接，非结构化网络使用闲话协议来保证随机性进而提供请求服务（提高容错率和平均化节点之间的负载但是却会带来高overhead和延时），感觉这些方案都不如结构化来得好。
* 结构化虚拟网络进行内容分发通常有两种策略：
  1. DHT-Home DHT找到的节点就储存着 请求内容。对于一些热点内容，节点之间会随着请求的增加而不断重复。
  2. DHT-Directory DHT找到的节点储存着的是 储存着该内容的节点指针列表，需要进一步访问这些节点指针才能获取到所需要的请求内容。
* 总的来说，结构化虚拟网络利用了DHT的负载均衡和重复特性可以达到自扩展性，但是这样的请求路由有两个缺陷，第一是根据DHT路由必须在不同节点之间跳跃，在公司局域网中这样路由跳转带来的延时还能接受，但是在公网中的话使用CS架构就会快得多。第二是除非使用考虑到空间位置或者主动复制的结构化虚拟网络，请求会随机地分散给p2p系统中节点。
* 文中之后介绍了数种不同的结构化和非结构化耦合在一起的虚拟网络的架构，例如Kache，Flower-CDN，PetalUp-CDN，这里就不赘述了。在非结构网络中用目录储存节点索引并使用gossip分享和重复索引信息是他们的共同点。
* 这章最末尾通过比较这些不同的网络结构得出来的观察结果是大多数解决方案并不聚焦在可扩展性上，而是针对于小型的本地网络。

## 第三章 推荐系统
。。。to be continued。在阅读这部分内容之前，希望能代码实现一个分布式哈希表:)加油！
